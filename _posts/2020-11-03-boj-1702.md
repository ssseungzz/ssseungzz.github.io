...
그래프의 정점의 집합을 둘로 분할하여, 각 집합에 속한 정점끼리는 서로 인접하지 않도록 분할할 수 있을 때, 그러한 그래프를 특별히 이분 그래프라고 부른다. 그래프가 입력으로 주어졌을 때 이 그래프가 이분 그래프인지 아닌지 판별하는 프로그램을 작성하는 문제이다. 가장 흔한 풀이 방식으로 정점마다 DFS 혹은 BFS를 돌려 탐색하면서 방문 단계마다 두 가지 색을 번갈아가면서 칠해 주는 방식이 있다. 나는 이 방식으로 문제 풀이를 진행했다. 처음에는 그래프 배열에 색상 정보까지 저장하려고 하다가, 번거로워질 것 같아 따로 색상 배열을 만들어 주었다. BFS로 탐색을 진행하면서 부모 노드와 다른 색으로 색상 배열에 색상을 저장하도록 했다. 처음에는 문제를 틀렸는데, 그 이유는 연결 그래프를 가정하고 문제를 풀었기 때문이었다. 문제 어디에도 __연결 그래프만 주어진다__는 조건이 없으므로, 연결 그래프가 아닌 경우도 가정해 줘야 한다. 가령 아래와 같은 경우는 연결 그래프가 아니면서 이분 그래프인 경우이다.
